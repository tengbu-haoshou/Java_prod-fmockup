<template>
	<v-icon v-if="mode == 'assign'" large @click="popup('assign')">mdi-account-multiple-plus</v-icon>
	<v-icon v-if="mode == 'unassign'" large @click="popup('unassign')">mdi-account-multiple-minus</v-icon>
						...
						<v-col>
							<v-form>
								<v-container fluid>
									<v-row no-gutters>
										<v-col :cols="field_md">
											<v-text-field disabled autocomplete="off" prepend-icon="mdi-account" :label="text_form_userId" v-model="userName" />
										</v-col>
									</v-row>
									<v-row no-gutters>
										<v-col>
											<v-radio-group v-model="radio" hide-details="auto" v-for="(item, i) in items" :key="i">
												<v-radio v-if="mode == 'assign' && item.isExists == 'true'" disabled :label="item.roleName" :value="item.roleId" />
												<v-radio v-if="mode == 'assign' && item.isExists == 'false'" :label="item.roleName" :value="item.roleId" />
												<v-radio v-if="mode == 'unassign' && item.isExists == 'false'" disabled :label="item.roleName" :value="item.roleId" />
												<v-radio v-if="mode == 'unassign' && item.isExists == 'true'" :label="item.roleName" :value="item.roleId" />
											</v-radio-group>
										</v-col>
									</v-row>
									...
									<v-row v-if="mode == 'unassign'" no-gutters>&nbsp;</v-row>
									<v-row no-gutters>
										<v-col cols="auto">
...
<script setup>
	...
	function receive_texts(texts_status, texts_message, texts_value) {
		if (texts_status == 'action-ok') {
			...
			text_title_assign.value = texts_json.user_role_dialog_title_assign
			text_title_unassign.value = texts_json.user_role_dialog_title_unassign
			text_form_userId.value = texts_json.user_role_dialog_form_userId
			text_form_assign.value = texts_json.user_role_dialog_form_assign
			text_form_unassign.value = texts_json.user_role_dialog_form_unassign
			text_form_cancel.value = texts_json.user_role_dialog_form_cancel
			text_notice_sure.value = texts_json.user_role_dialog_notice_sure
		} else {
			display.value = 'error'
	...
	function popup(mode) {
		...
		axios.get(`/user_role/list?userId=${userId}&updateDate=${updateDate}`)
		.then (function(response) {
			if (response.data.status == 'action-ok') {
	...
	async function action(mode) {
		...
		formData.append('roleId', radio.value ? radio.value : '')
		await axios.post('/user_role/' + mode, formData)
		.then (function(response) {
			if (response.data.status == 'action-ok') {
