						...
						<v-col>
							<v-form>
								<v-container fluid>
									<v-row no-gutters>
										<v-col :cols="field_md">
											<div style="font-size: small; color: red;">{{ messages.yyyyName }}</div>
											<v-text-field v-if="disabled == true" disabled autocomplete="off" prepend-icon="mdi-account-multiple" :label="text_form_name" v-model="yyyyName" />
											<v-text-field v-if="disabled == false" autocomplete="off" prepend-icon="mdi-account-multiple" clearable :label="text_form_name" v-model="yyyyName" />
										</v-col>
									</v-row>
									<v-row no-gutters>
										<v-col :cols="field_lg">
											<div style="font-size: small; color: red;">{{ messages.yyyyFlag }}</div>
											<v-switch v-if="disabled == true" disabled prepend-icon="mdi-variable-box" :label="text_form_flag" v-model="yyyyFlag" />
											<v-switch v-if="disabled == false" prepend-icon="mdi-variable-box" clearable :label="text_form_flag" v-model="yyyyFlag" />
										</v-col>
									</v-row>
									<v-row no-gutters>
										<v-col :cols="field_md">
											<div style="font-size: small; color: red;">{{ messages.yyyyValue }}</div>
											<v-text-field v-if="disabled == true" disabled autocomplete="off" prepend-icon="mdi-variable-box" :label="text_form_value" v-model="yyyyValue" />
											<v-text-field v-if="disabled == false" autocomplete="off" prepend-icon="mdi-variable-box" clearable :label="text_form_value" v-model="yyyyValue" />
										</v-col>
									</v-row>
									<v-row no-gutters>
										<v-col :cols="field_lg">
...
<script setup>
	...
	function receive_texts(texts_status, texts_message, texts_value) {
		if (texts_status == 'action-ok') {
			...
			text_title_add.value = texts_json.yyyy_dialog_title_add
			text_title_modify.value = texts_json.yyyy_dialog_title_modify
			text_title_drop.value = texts_json.yyyy_dialog_title_drop
			text_form_name.value = texts_json.yyyy_dialog_form_name
			text_form_flag.value = texts_json.yyyy_dialog_form_flag
			text_form_value.value = texts_json.yyyy_dialog_form_value
			text_form_remark.value = texts_json.yyyy_dialog_form_remark
			text_form_add.value = texts_json.yyyy_dialog_form_add
			text_form_modify.value = texts_json.yyyy_dialog_form_modify
			text_form_drop.value = texts_json.yyyy_dialog_form_drop
			text_form_cancel.value = texts_json.yyyy_dialog_form_cancel
			text_notice_sure.value = texts_json.yyyy_dialog_notice_sure
		} else {
			display.value = 'error'
	...
	function popup(mode) {
		...
		if (mode == 'modify' || mode == 'drop') {
			const yyyyId = props.yyyyId
			const updateDate = props.updateDate
			axios.get(`/yyyy/info?yyyyId=${yyyyId}&updateDate=${updateDate}`)
			.then (function(response) {
				if (response.data.status == 'action-ok') {
					yyyyName.value = response.data.messageMap.yyyyName
					yyyyFlag.value = (response.data.messageMap.yyyyFlag === 'Y' ? true : false)
					yyyyValue.value = response.data.messageMap.yyyyValue
	...
	async function action(mode) {
		...
		formData.append('yyyyName', yyyyName.value != null ? yyyyName.value : '')
		if (mode == 'add' || mode == 'modify') {
			formData.append('yyyyFlag', yyyyFlag.value ? 'Y' : 'N')
			formData.append('yyyyValue', yyyyValue.value != null ? yyyyValue.value : '')
			formData.append('remark', remark.value != null ? remark.value : '')
		}
		if (mode == 'modify' || mode == 'drop') {
			formData.append('yyyyId', props.yyyyId)
			formData.append('updateDate', props.updateDate)
		}
		await axios.post('/yyyy/' + mode, formData)
		.then (function(response) {
			if (response.data.status == 'action-ok') {
				window.location.href = '/pages/yyyy_list.html'
				return
			} else {
				message.value = response.data.message
				if (mode != 'drop') {
