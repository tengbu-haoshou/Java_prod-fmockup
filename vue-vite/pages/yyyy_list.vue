			...
			<v-main>
				<v-container fluid>
					<v-row no-gutters>
						<v-col cols="12">
							<v-tabs v-model="tab">
								<v-tab style="text-transform: none" href="/pages/yyyy_list.html" value="YyyyList"><h3 class="display-1">{{ text_tab_yyyy_list }}</h3></v-tab>
							</v-tabs>
						</v-col>
					</v-row>
					<v-row no-gutters>
					...
					<v-row>
						<v-col>
							<v-data-table :headers="headers" :items="items">
								<template v-slot:item.yyyyId="{ item }" />			<!-- to hide -->
								<template v-slot:item.updateDate="{ item }" />		<!-- to hide -->
								<template v-slot:item.no="{ item }">
									<div align="right">{{ item.no }}</div>
								</template>
								<template v-slot:item.yyyyName="{ item }">
									<div align="left">{{ item.yyyyName }}</div>
								</template>
								<template v-slot:item.yyyyValue="{ item }">
									<div align="left">{{ item.yyyyValue }}</div>
								</template>
								<template v-slot:item.remark="{ item }">
									<div align="left">{{ item.remark }}</div>
								</template>
								<template v-slot:item.actions="{ item }">
									<dialog_yyyy v-if="item.deletedF == 'N'" :mode="modify_mode" :token="token" :yyyyId="item.yyyyId" :updateDate="item.updateDate" />
									<dialog_yyyy v-if="item.deletedF == 'N'" :mode="drop_mode" :token="token" :yyyyId="item.yyyyId" :updateDate="item.updateDate" />
								</template>
								<template v-slot:top>
									<dialog_yyyy :mode="add_mode" :token="token" :yyyyId="space" :updateDate="space" />
								</template>
							</v-data-table>
						</v-col>
					</v-row>
...
<script setup>
	...
	const hover = ref('YyyyList')
	const message = ref('')

	const loading = ref(true)
	const display = ref('')
	const tab = ref('YyyyList')
	...
	function receive_texts(texts_status, texts_message, texts_value) {
		if (texts_status == 'action-ok') {
			...
			text_tab_yyyy_list.value = texts_json.yyyy_list_tab_yyyy_list
		} else {
			display.value = 'error'
	...
	import dialog_yyyy from './dialogs/yyyy.vue'

	const headers = [
//		{title: 'Yyyy ID (Hidden)', key: 'yyyyId', align: 'center'},				// hidden
//		{title: 'Update Datetime (Hidden)', key: 'updateDate', align: 'center'},	// hidden
		{title: 'No.', key: 'no', align: 'center'},
		{title: 'Actions', key: 'actions', align: 'center', sortable: false },
		{title: 'Yyyy Name', key: 'yyyyName', align: 'center'},
		{title: 'Deteted', key: 'deletedF', align: 'center'},
		{title: 'Yyyy Flag', key: 'yyyyFlag', align: 'center'},
		{title: 'Yyyy Value', key: 'yyyyValue', align: 'center'},
		{title: 'Remark', key: 'remark', align: 'center'},
		{title: 'Modify Datetime', key: 'modifyYyyyDate', align: 'center'},
		{title: 'Add Datetime', key: 'addYyyyDate', align: 'center'},
	]

	const formData = new FormData()
	axios.get('/yyyy/list', formData)
