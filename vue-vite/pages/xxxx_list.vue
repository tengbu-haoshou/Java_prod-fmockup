			...
			<v-main>
				<v-container fluid>
					<v-row no-gutters>
						<v-col cols="12">
							<v-tabs v-model="tab">
								<v-tab style="text-transform: none" href="/pages/xxxx_list.html" value="XxxxList"><h3 class="display-1">{{ text_tab_xxxx_list }}</h3></v-tab>
							</v-tabs>
						</v-col>
					</v-row>
					<v-row no-gutters>
					...
					<v-row>
						<v-col>
							<v-data-table :headers="headers" :items="items">
								<template v-slot:item.xxxxId="{ item }" />			<!-- to hide -->
								<template v-slot:item.updateDate="{ item }" />		<!-- to hide -->
								<template v-slot:item.no="{ item }">
									<div align="right">{{ item.no }}</div>
								</template>
								<template v-slot:item.xxxxName="{ item }">
									<div align="left">{{ item.xxxxName }}</div>
								</template>
								<template v-slot:item.xxxxValue="{ item }">
									<div align="left">{{ item.xxxxValue }}</div>
								</template>
								<template v-slot:item.remark="{ item }">
									<div align="left">{{ item.remark }}</div>
								</template>
								<template v-slot:item.actions="{ item }">
									<dialog_xxxx v-if="item.deletedF == 'N'" :mode="modify_mode" :token="token" :xxxxId="item.xxxxId" :updateDate="item.updateDate" />
									<dialog_xxxx v-if="item.deletedF == 'N'" :mode="drop_mode" :token="token" :xxxxId="item.xxxxId" :updateDate="item.updateDate" />
								</template>
								<template v-slot:top>
									<dialog_xxxx :mode="add_mode" :token="token" :xxxxId="space" :updateDate="space" />
								</template>
							</v-data-table>
						</v-col>
					</v-row>
...
<script setup>
	...
	const hover = ref('XxxxList')
	const message = ref('')

	const loading = ref(true)
	const display = ref('')
	const tab = ref('XxxxList')
	...
	function receive_texts(texts_status, texts_message, texts_value) {
		if (texts_status == 'action-ok') {
			...
			text_tab_xxxx_list.value = texts_json.xxxx_list_tab_xxxx_list
		} else {
			display.value = 'error'
	...
	import dialog_xxxx from './dialogs/xxxx.vue'

	const headers = [
//		{title: 'Xxxx ID (Hidden)', key: 'xxxxId', align: 'center'},				// hidden
//		{title: 'Update Datetime (Hidden)', key: 'updateDate', align: 'center'},	// hidden
		{title: 'No.', key: 'no', align: 'center'},
		{title: 'Actions', key: 'actions', align: 'center', sortable: false },
		{title: 'Xxxx Name', key: 'xxxxName', align: 'center'},
		{title: 'Deteted', key: 'deletedF', align: 'center'},
		{title: 'Xxxx Flag', key: 'xxxxFlag', align: 'center'},
		{title: 'Xxxx Value', key: 'xxxxValue', align: 'center'},
		{title: 'Remark', key: 'remark', align: 'center'},
		{title: 'Modify Datetime', key: 'modifyXxxxDate', align: 'center'},
		{title: 'Add Datetime', key: 'addXxxxDate', align: 'center'},
	]

	const formData = new FormData()
	axios.get('/xxxx/list', formData)
